...
public:
    ...
    LogStream(std::shared_ptr<LogConsumer>& logConsumer, const std::shared_ptr<LogConfig>& cfg);
    ~LogStream();
    ...
    {
        constexpr const char* str[] = {"\033[0;32m[INFO]\033[0m", "\033[0;33m[WARN]\033[0m", "\033[0;31m[ERROR]\033[0m",
...
#define LOG_CONDITATION(level) (static_cast<int>((level)) >= static_cast<int>(logger::LogStream::instance().getLevel()))
...
template <typename T>
struct HasToStringFunc<T, VoidType<decltype(std::stringstream() << std::declval<T>())>> : public std::true_type
{
};

template <typename T>
inline LogStream& WriteToLoggerStream(LogStream& s, T&& t, std::enable_if_t<HasToStringFunc<T>::value>* = nullptr)
{
...
struct LogWrapper
{
    explicit LogWrapper(LogLevel level) : level_(level)
    {
        ...
        LogStream::instance() << LogStream::instance().getStrLevel(level) << "[TID:" << LogStream::threadId() << "]";
    }
...
struct SimpleStringRef
{
    ...
    static constexpr size_t alignSize()
    {
    ...
    SimpleStringRef* next()
    {
        ...
        return reinterpret_cast<SimpleStringRef*>(reinterpret_cast<char*>(this) + objSize());
    }
...
inline std::ostream& operator<<(std::ostream& os, const SimpleStringRef& strRef)
{
...
public:
    ...
    public:
        ...
        const SimpleStringRef& operator*() const
        {
            return *ptr_;
        }
        const SimpleStringRef* operator->() const
    ...
    auto pool_begin()
    {
        return reinterpret_cast<char*>(pool_);
    }
    auto pool_end()
    {
        return reinterpret_cast<char*>(currentPoolEnd_);
    }
    ...
    void setFlushFunc(const FlushFunc& flushFunc)
    {
