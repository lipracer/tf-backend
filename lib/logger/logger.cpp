...
{
    return alignUp(size + 1 + SimpleStringRef::headerSize(), SimpleStringRef::alignSize());
}
...
SimpleStringRef::SimpleStringRef(const char* str, size_t size) : size_(size)
{
    if (size == 0)
    {
        memset(reinterpret_cast<char*>(this) + SimpleStringRef::headerSize(), 0, SimpleStringRef::alignSize());
    }
    else
    {
        memcpy(reinterpret_cast<char*>(this) + SimpleStringRef::headerSize(), str,
               objSize() - SimpleStringRef::headerSize());
    }
}

size_t SimpleStringRef::objSize() const
{
    return alignStringSize(size_);
}
...
SimpleStringRef* SimpleStringRef::create(StringPool& pool, const char* str, size_t size)
{
    auto buf = pool.allocStringBuf();
    auto strRef = new (buf) logger::SimpleStringRef(str, size);
    assert(pool.pool_begin() <= buf && buf < pool.pool_end() && "buf not in range!");
    return strRef;
}

#define CHECK_BEGIN_IN_RANGE()                                                                               \
    assert(pool_begin() <= reinterpret_cast<char*>(front()) && reinterpret_cast<char*>(front()) < pool_end() \
...
StringPool::StringPool(size_t pageSzie, const FlushFunc& flush) : flushFunc_(flush), pageSize_(pageSzie)
{
...
char* StringPool::allocStringBuf()
{
...
size_t StringPool::debugSize()
{
    ...
    assert(reinterpret_cast<char*>(&*iter) <= pool_begin() + pageSize_ && "offset error!");
    return i;
}
    ...
    void print()
    {
        ...
        while (!exit_ || buf_.size())
        {
            ...
            else
            {
...
#else
                mtx_.lock();
                size_t consumeSize = buf_.size() <= tmpBuffer_.size() ? buf_.size() : tmpBuffer_.size();
                std::copy(buf_.begin(), buf_.begin() + consumeSize, tmpBuffer_.begin());
                ...
                for (size_t i = 0; i < consumeSize; ++i)
                {
                    fwriteString(tmpBuffer_[i].c_str(), tmpBuffer_[i].size(), cfg_->stream);
                }
#endif
            }
        LOOP_END:
...
LogStream& LogStream::instance(const LogConfig& cfg)
{
    ...
    static thread_local LogStream* __instance = new LogStream(gLogConsumer, sp_cfg);
    gLogConsumer->notify();
    return *__instance;
}
...
LogStream::LogStream(std::shared_ptr<LogConsumer>& logConsumer, const std::shared_ptr<LogConfig>& cfg)
...
void LogStream::log_fatal()
{
    logConsumer_->report_fatal();
}

void initLogger(const LogConfig& cfg)
{
    (void)LogStream::instance(cfg);
}

thread_local std::chrono::high_resolution_clock::duration LogWrapper::totalDur{};
